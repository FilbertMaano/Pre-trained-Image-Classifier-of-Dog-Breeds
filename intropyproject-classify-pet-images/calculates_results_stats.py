...
def calculates_results_stats(results_dic):
    ...
    n_correct_dogs = n_dogs_img = n_correct_notdogs = 0
    n_notdogs_img = n_correct_breed = n_match = 0

    for value in results_dic.values():
       match = value[2]
       is_a_dog, classified_as_a_dog = value[3], value[4]
 
       if is_a_dog and classified_as_a_dog:
          n_correct_dogs += 1
       if is_a_dog:
          n_dogs_img += 1
       if not is_a_dog and not classified_as_a_dog:
          n_correct_notdogs += 1
       if not is_a_dog:
          n_notdogs_img += 1
       if is_a_dog and match:
          n_correct_breed += 1
       if match:
          n_match += 1
